...
const phaseDefinitions: PhaseDefinition[] = [
  {
    ...
    phaseType: 'planning',
    goal: 'Lock the repo onto a production application target and document what MVP Builder must deliver in this release.',
  ...
  },
  {
    ...
    goal: 'Validate the markdown package generator, templates, and type model that power MVP Builder.',
    ...
    repoFiles: ['lib/generator.ts', 'lib/templates.ts', 'lib/types.ts', 'scripts/mvp-builder-create-project.ts'],
  ...
  },
  {
    ...
    phaseType: 'implementation',
    goal: 'Make repo-root status, validate, scoring, and phase-state behavior work as a real MVP Builder package lifecycle.',
    ...
    repoFiles: ['scripts/mvp-builder-status.ts', 'scripts/mvp-builder-validate.ts', 'scripts/mvp-builder-package-utils.ts', 'repo/mvp-builder-state.json'],
    commands: ['repo/release-evidence/commands/status.md', 'repo/release-evidence/commands/validate.md'],
    evidenceFiles: ['repo/release-evidence/phases/phase-08-evidence.md'],
    changedFiles: ['scripts/mvp-builder-status.ts', 'repo/manifest.json', 'repo/mvp-builder-state.json', 'package.json'],
    testsRun: ['npm run status', 'npm run validate'],
    knownIssues: [],
  ...
  },
  {
    ...
    summary: 'The gate runner now exposes the named gates required for a production MVP Builder release, and the repo produces score and gate reports from real commands.',
    repoFiles: ['lib/orchestrator/gates.ts', 'lib/orchestrator/score.ts', 'lib/orchestrator/recovery.ts', 'scripts/mvp-builder-gates.ts'],
  ...
  },
  {
    ...
    repoFiles: ['FINAL_RELEASE_REPORT.md', 'FINAL_HANDOFF.md', 'FINAL_DEPLOYMENT_STATUS.md', 'repo/manifest.json', 'repo/mvp-builder-state.json'],
    ...
    changedFiles: ['FINAL_RELEASE_REPORT.md', 'FINAL_HANDOFF.md', 'FINAL_DEPLOYMENT_STATUS.md', 'repo/manifest.json', 'repo/mvp-builder-state.json'],
...
## Observed result
- ${phase.summary}
- The referenced repo files exist in the current workspace and align with the production release target for MVP Builder.
...
This package contains ${phaseDefinitions.length} phases for MVP Builder.
...
## Release objective
- Release the validated local-first MVP Builder repo as a production-ready developer tool and handoff package.
...
- Verify FINAL_* docs and repo/mvp-builder-state.json remain synchronized
...
- [x] repo/mvp-builder-state.json and repo/manifest.json reflect final lifecycle completion.
...
## Service overview
- MVP Builder is a local-first Next.js and Node.js toolchain for generating, validating, scoring, and handing off markdown-based project packages.
...
- Compare repo/mvp-builder-state.json, FINAL_RELEASE_REPORT.md, and orchestrator reports for drift.
...
- Lifecycle state: repo/manifest.json and repo/mvp-builder-state.json
...
function buildManifest() {
  ...
  return {
    exportRoot: 'mvp-builder-workspace',
    ...
    packageSummary: 'Production-ready MVP Builder repo package with synchronized lifecycle evidence.',
    warningCounts: {
      info: 0,
      warning: 0,
      blocker: 0
...
function main() {
  ...
  writeFile('repo/mvp-builder-state.json', JSON.stringify(buildState(), null, 2));
}
