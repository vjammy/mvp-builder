# MVP Builder Autoresearch Program

The MVP Builder Autoresearch Program is the MVP Builder's self-evaluation harness. It exists so the MVP Builder repo can keep proving, run after run, that:
...
2. Runs `npx tsx scripts/mvp-builder-validate.ts --package=<root>`.
...
- `autoresearch/MVP_BUILDER_AUTORESEARCH_PROGRAM.md` (this file) — program description.
