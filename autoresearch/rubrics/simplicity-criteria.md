...
The Manoa Method is local-first, markdown-first, and intentionally small. The autoresearch program enforces that by treating overbuild as a quality regression, not a feature.
...
- Does **not** push the user toward auth, payments, or analytics services that the Manoa Method explicitly disclaims providing.
...
For a Manoa-generated package, simple means:
...
- No surprise auth / payments / analytics: the Manoa Method does not provide these and should not pretend to.
...
In the Manoa Method's first months, several generator "improvements" added complexity that:
...
- Implied features (auth, payments, dashboards) that the Manoa Method explicitly disclaims.
